...
public class BudgetService {
    ...
    public Budget createBudget(String userId, @Valid BudgetRequest request) {
        ...
        // Check if a budget already exists for this user, period, and category
        if (category != null && !category.isEmpty()) {
            budgetRepository.findByUserIdAndPeriodAndCategory(userId, period, category).ifPresent(existingBudget -> {
                // Delete the existing budget to overwrite it
                budgetRepository.delete(existingBudget);
            });
        } else {
            budgetRepository.findFirstByUserIdAndPeriod(userId, period).ifPresent(existingBudget -> {
                // Delete the existing budget to overwrite it if it has no category
                if (existingBudget.getCategory() == null || existingBudget.getCategory().isEmpty()) {
                    budgetRepository.delete(existingBudget);
                }
            });
        }

        Budget budget = new Budget(userId, request.getAmount(), period, request.getCategory());
    ...
    private String normalizePeriod(String period) {
        ...
        throw new IllegalArgumentException(
                "Invalid period format. Use 'MONTHLY' or 'YYYY-MM' format (e.g., '2026-02')");
    }
    ...
    private BudgetStatus calculateBudgetStatus(Budget budget, String userId) {
        ...
        // Calculate spent amount - use category-specific spending if budget has a
        // category
        BigDecimal spent;
        if (budget.getCategory() != null && !budget.getCategory().isEmpty()) {
            spent = transactionRepository.getTotalSpendingByCategory(userId, budget.getCategory(), startDate, endDate);
        } else {
            spent = transactionRepository.getTotalSpending(userId, startDate, endDate);
        }
    ...
    public void recalculateBalance(String userId) {
        try {
            ...
            if (budgets.isEmpty()) {
                throw new NoSuchElementException("User doesn't have a budget yet");
            }

            // Process each budget (including category-specific ones)
            for (Budget budget : budgets) {
                ...
                BigDecimal spent;
                if (budget.getCategory() != null && !budget.getCategory().isEmpty()) {
                    System.out.println("Querying spending for category: " + budget.getCategory());
                    System.out.println("Date range: " + startDate + " to " + endDate);
                    spent = transactionRepository.getTotalSpendingByCategory(userId, budget.getCategory(), startDate,
                            endDate);
                    System.out.println("Query result (spent): " + spent);
                } else {
                    spent = transactionRepository.getTotalSpending(userId, startDate, endDate);
                }
                ...
                System.out.println("Category: " + (budget.getCategory() != null ? budget.getCategory() : "All"));
